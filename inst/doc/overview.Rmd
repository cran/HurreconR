---
title: "Overview of HurreconR"
date: "12 June 2023"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Overview of HurreconR}
...
1. Install and load the R package for the model (HurreconR).
...
Examples of input files may be found on the <i>inst/input</i> subdirectory.

To run the model, create the above directories, copy the input files to their
respective subdirectories, and use the library function to load HurreconR.
...
<i>region</i>, <i>region-dt</i>, or <i>region-all</i> subdirectory as CSV or GeoTiff 
files. The default time step for site results is 1 minute. The default time step for 
regional results is calculated as the time required to traverse one cell in the 
...
for a single hurricane and a single site. The <i>hurrecon_summarize_region</i> function
